...
final class NotificationProviderHealthSlaRiskPolicyAuditNotificationDeliveryHealthAlertEscalationDeliveryReliabilityTrendHealthActionAudit {
    ...
    public static function record(string $action, array $health, array $state): void {
        ...
        $entries[] = [
            ...
            'status' => sanitize_key((string)($health['status'] ?? 'no-data')),
            'success_rate' => (float)($health['success_rate'] ?? 0),
            'retry_rate' => (float)($health['retry_rate'] ?? 0),
            'failure_rate' => (float)($health['failure_rate'] ?? 0),
            'snapshot_count' => (int)($health['snapshot_count'] ?? 0),
            'action_count' => (int)($state['action_count'] ?? 0),
        ];

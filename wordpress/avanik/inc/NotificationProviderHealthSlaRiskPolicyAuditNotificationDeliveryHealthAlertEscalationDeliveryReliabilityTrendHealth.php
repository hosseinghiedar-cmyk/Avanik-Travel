...
final class NotificationProviderHealthSlaRiskPolicyAuditNotificationDeliveryHealthAlertEscalationDeliveryReliabilityTrendHealth {
    ...
    public static function assess(): array {
        ...
        if ($latest) {
            $failure = (float)$latest['failure_rate'];
            $retry = (float)$latest['retry_rate'];
            $success = (float)$latest['success_rate'];
            $status = 'stable';
            $reason = 'Current delivery reliability is within the normal bounded trend state.';
            if ($failure >= 20 || $success < 80 || $retry >= 30) {
                $status = 'degraded';
                $reason = 'Failure, retry, or success-rate thresholds indicate degraded escalation delivery reliability.';
            } elseif ($previous) {
                $failureDelta = $failure - (float)$previous['failure_rate'];
                $successDelta = $success - (float)$previous['success_rate'];
                if ($failureDelta >= 10 || $successDelta <= -10) {
                    $status = 'degrading';
                    $reason = 'The latest snapshot is materially worse than the previous snapshot.';
                } elseif ($failureDelta <= -10 || $successDelta >= 10) {
                    $status = 'improving';
                    $reason = 'The latest snapshot is materially better than the previous snapshot.';
                }
            }
        }
        $assessment = [
            'status' => $status,
            'reason' => $reason,
            'latest_at' => $latest ? (int)$latest['at'] : 0,
            'failure_rate' => $latest ? (float)$latest['failure_rate'] : 0.0,
            'retry_rate' => $latest ? (float)$latest['retry_rate'] : 0.0,
            'success_rate' => $latest ? (float)$latest['success_rate'] : 0.0,
    ...
    public static function render(): void {
        ...
        $rows = [
            ...
            'Retry rate' => $a['retry_rate'].'%',
            'Failure rate' => $a['failure_rate'].'%',
            'Snapshots' => $a['snapshot_count'],
            'Latest snapshot' => $a['latest_at'] ? wp_date('Y-m-d H:i:s', $a['latest_at']) : '—',
        ];

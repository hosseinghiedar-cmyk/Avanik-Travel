...
final class NotificationProviderHealthSlaRiskPolicyAuditNotificationDeliveryHealthAlertEscalationDeliveryReliabilityTrendHealthActionAuditExportIntegrity {
    ...
    public static function build_csv(): string {
        ...
        foreach ($entries as $entry) {
            fputcsv($handle, [wp_date('c', (int)$entry['at']), (string)$entry['action'], (string)$entry['status'], (float)$entry['success_rate'], (float)$entry['retry_rate'], (float)$entry['failure_rate'], (int)$entry['snapshot_count'], (int)$entry['action_count']]);
        }
        rewind($handle);
    ...
    public static function checksum(): string {
        return hash('sha256', self::build_csv());
    }

    public static function render(): void {
        if (!current_user_can(self::CAPABILITY)) return;
        $entries = NotificationProviderHealthSlaRiskPolicyAuditNotificationDeliveryHealthAlertEscalationDeliveryReliabilityTrendHealthActionAudit::entries();
        echo '<div class="wrap"><h1>SLA Trend Health Audit Export Integrity</h1><p>This checksum is calculated from the exact bounded audit dataset used by the Phase 102 CSV export.</p>';
        echo '<table class="widefat striped"><tbody>';
        echo '<tr><th>Entries</th><td>'.count($entries).'</td></tr>';
        echo '<tr><th>CSV SHA-256</th><td><code>'.esc_html(self::checksum()).'</code></td></tr>';

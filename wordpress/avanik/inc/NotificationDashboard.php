...
final class NotificationDashboard {
 ...
 public static function menu(): void { add_submenu_page('avanik-theme-settings','مرکز اعلان آوانیک','مرکز اعلان‌ها','manage_options','avanik-notification-dashboard',[self::class,'page']); }
 public static function page(): void {
  ...
  echo '<div class="wrap" dir="rtl"><h1>مرکز اعلان‌های آوانیک</h1><form method="get"><input type="hidden" name="page" value="avanik-notification-dashboard"><label>بازه زمانی <select name="days">'; foreach([1,7,14,30,90] as $d)echo '<option value="'.$d.'" '.selected($days,$d,false).'>'.$d.' روز اخیر</option>'; echo '</select></label> <button class="button">اعمال</button></form><div style="display:grid;grid-template-columns:repeat(4,minmax(160px,1fr));gap:12px;margin:18px 0">'; self::card('در صف / تلاش مجدد',$queued); self::card('ناموفق',$failed); self::card('متوقف‌شده',$dead); self::card('خوانده‌نشده',$unread); echo '</div><div style="display:grid;grid-template-columns:repeat(3,minmax(240px,1fr));gap:20px"><section><h2>وضعیت</h2>'; self::table(['وضعیت','تعداد'],$qstats,'status'); echo '</section><section><h2>کانال</h2>'; self::table(['کانال','تعداد'],$cstats,'channel'); echo '</section><section><h2>نقش</h2>'; self::table(['نقش','تعداد'],$rstats,'role'); echo '</section></div><p><strong>محدوده گزارش:</strong> آمار فقط مربوط به بازه انتخاب‌شده است.</p></div>'; }

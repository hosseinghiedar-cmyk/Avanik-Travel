...
final class NotificationProviderHealthSlaRiskPolicyAuditNotificationDeliveryHealthAlertEscalationDeliveryReliabilityTrendHealthActionAuditExportVerificationReportScheduleHealth {
    ...
    public static function state(): array {
        ...
        return [
            ...
            'last_refresh' => $last,
            'age_seconds' => $last ? max(0, time() - $last) : null,
            'healthy' => (bool)$next && $last > 0 && (time() - $last) <= (2 * HOUR_IN_SECONDS),
        ];
    }
    ...
    public static function render(): void {
        ...
        $rows = [
            'Status' => $s['healthy'] ? 'HEALTHY' : 'ATTENTION',
            'Cron scheduled' => $s['scheduled'] ? 'YES' : 'NO',
            'Next run' => $s['next_run'] ? wp_date('Y-m-d H:i:s', $s['next_run']) : '—',
            'Last refresh' => $s['last_refresh'] ? wp_date('Y-m-d H:i:s', $s['last_refresh']) : '—',
            'Refresh age' => $s['age_seconds'] === null ? '—' : ((int)$s['age_seconds']).' seconds',
        ];

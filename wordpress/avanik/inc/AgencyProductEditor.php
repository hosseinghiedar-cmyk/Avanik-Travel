...
final class AgencyProductEditor {
  ...
  public static function render(): string {
    ...
    $meta=json_decode((string)$p['metadata'],true)?:[]; $selected=PassengerRequirements::for_product($p); $labels=PassengerRequirements::labels(); ob_start(); ?>
    <form method="post" action="<?php echo esc_url(admin_url('admin-post.php')); ?>" dir="rtl">
      ...
      <p><label>نوع <select name="type"><?php foreach(['tour'=>'تور','hotel'=>'هتل','flight'=>'پرواز','package'=>'پکیج','domestic_flight'=>'پرواز داخلی','international_flight'=>'پرواز خارجی'] as $k=>$v): ?><option value="<?php echo esc_attr($k); ?>" <?php selected($p['type'],$k); ?>><?php echo esc_html($v); ?></option><?php endforeach; ?></select></label></p>
      ...
      <fieldset><legend>اطلاعات موردنیاز هر مسافر</legend><?php foreach($labels as $key=>$label): ?><label style="display:block;margin:6px 0"><input type="checkbox" name="passenger_fields[]" value="<?php echo esc_attr($key); ?>" <?php checked(in_array($key,$selected,true)); ?>> <?php echo esc_html($label); ?></label><?php endforeach; ?><small>برای پرواز خارجی فیلدهای پاسپورت و تاریخ انقضا را فعال کنید. فیلدهای پایه همیشه نگه داشته می‌شوند.</small></fieldset>
  ...
  public static function save(): void {
    ...
    $fields=(array)($_POST['passenger_fields']??[]);
    $metadata=['destination'=>sanitize_text_field(wp_unslash($_POST['destination']??'')),'start_date'=>sanitize_text_field(wp_unslash($_POST['start_date']??'')),'end_date'=>sanitize_text_field(wp_unslash($_POST['end_date']??'')),'description'=>sanitize_textarea_field(wp_unslash($_POST['description']??'')),'passenger_fields'=>PassengerRequirements::normalize($fields)];
    ProductRepository::update_owned((int)($_POST['product_id']??0),$u->ID,['type'=>wp_unslash($_POST['type']??Product::TOUR),'title'=>wp_unslash($_POST['title']??''),'price'=>(float)($_POST['price']??0),'currency'=>wp_unslash($_POST['currency']??'IRR'),'capacity'=>(int)($_POST['capacity']??0),'metadata'=>$metadata]);

...
final class NotificationProviderHealthAlert {
 ...
 public static function evaluate(string $provider,array $summary,array $context=[]): array {
  ...
  $last=(string)($context['last_alert_code']??'');
  foreach($alerts as &$alert){$alert['provider']=$provider;$alert['dedupe_key']=$provider.':'.$alert['code'];$alert['repeat']=($last===$alert['code']);} unset($alert);
  return ['provider'=>$provider,'alerts'=>$alerts,'has_alert'=>!empty($alerts)];
 }
}
...
final class NotificationProviderHealthAlert {
 ...
 public static function evaluate(string $provider,array $summary,array $context=[]): array {
  ...
  $recovery=apply_filters('avanik_notification_provider_health_recovery',null,$provider,$summary,$context); $last=(string)($context['last_alert_code']??''); $out=[];
  ...
  return ['provider'=>$provider,'alerts'=>$out,'has_alert'=>!empty($out),'recovery'=>$recovery];
 }
}
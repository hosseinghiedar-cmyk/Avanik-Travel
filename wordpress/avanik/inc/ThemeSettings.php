...
final class ThemeSettings {
  ...
  private const MENU = 'avanik-theme-settings';
  private const OLD_SLUGS = ['avanik-theme-settings', 'avanik-settings', 'avanik-theme-options'];

  public static function register(): void {
    add_action('admin_menu', [self::class, 'admin_menu'], 20);
    add_action('admin_menu', [self::class, 'remove_legacy_settings_menu'], 999);
  ...
  public static function render(): void {
    ...
    <div class="wrap av-admin-wrap" dir="rtl">
      <style>
        .av-admin-wrap{max-width:1100px;font-family:Tahoma,"Vazirmatn",Arial,sans-serif}.av-admin-hero{background:linear-gradient(135deg,#072B5A,#0b447f);color:#fff;border-radius:18px;padding:28px 32px;margin:20px 0;display:flex;align-items:center;justify-content:space-between;gap:25px;box-shadow:0 14px 35px rgba(7,43,90,.18)}
  ...
  public static function guide(): void {
    if (!current_user_can('manage_options')) return;
    echo '<div class="wrap av-admin-wrap" dir="rtl"><h1>راهنمای قالب آوانیک</h1><div class="av-admin-card"><h2>مدیریت قالب</h2><ol class="av-admin-list"><li>از منوی «آوانیک» وارد «تنظیمات قالب» شوید.</li><li>رنگ‌ها، متن Hero، شماره تماس و رفتارهای نمایشی را تغییر دهید.</li><li>برای تغییرات ظاهری نیازی به ورود به «تنظیمات» عمومی وردپرس نیست.</li><li>قالب برای PHP 8.3 و WordPress جدید آماده شده است.</li></ol></div></div>';
  }

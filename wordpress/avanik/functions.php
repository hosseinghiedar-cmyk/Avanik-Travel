...
function avanik_jalali_date($gy,$gm,$gd){
    $g=[0,31,59,90,120,151,181,212,243,273,304,334];$gy2=($gm>2)?$gy+1:$gy;
    $d=355666+365*$gy+intdiv($gy2+3,4)-intdiv($gy2+99,100)+intdiv($gy2+399,400)+$gd+$g[$gm-1];
    $jy=-1595+33*intdiv($d,12053);$d%=12053;$jy+=4*intdiv($d,1461);$d%=1461;
    if($d>365){$jy+=intdiv($d-1,365);$d=($d-1)%365;}
    $jm=$d<186?1+intdiv($d,31):7+intdiv($d-186,30);$jd=1+($d<186?$d%31:($d-186)%30);

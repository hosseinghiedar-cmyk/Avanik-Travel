...
final class PassengerRequirements {
 ...
 public static function for_product(array $product): array { $type=sanitize_key($product['type']??$product['product_type']??''); $meta=json_decode((string)($product['metadata']??''),true)?:[]; $custom=self::normalize((array)($meta['passenger_fields']??[])); if($type===self::DOMESTIC_FLIGHT)return self::requirements($custom,$type); if($type===self::INTERNATIONAL_FLIGHT)return self::requirements($custom,$type); return self::normalize($custom); }

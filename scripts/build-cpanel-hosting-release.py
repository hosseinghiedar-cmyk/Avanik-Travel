...
def download_source_archive():
    ...
    archive=ROOT/'.hosting-source.tar.gz'
    req=urllib.request.Request(url,headers={'User-Agent':'avanik-cpanel-release-builder'})
...
def verify_theme():
    required=['style.css','functions.php','header.php','footer.php','front-page.php','inc/PhaseLoader.php','inc/Theme.php','inc/ThemeSetup.php','inc/ThemeSettings.php','assets/css/avanik-theme.css','assets/js/avanik-demo.js','assets/images/avanik-logo.svg','assets/images/avanik-hero.svg']
    for rel in required:
    ...
    if 'ThemeSetup.php' not in text or 'ThemeSetup::register' not in text:raise SystemExit('Demo ThemeSetup bootstrap is missing')
    if 'Theme.php' not in text or 'Theme::boot' not in text:raise SystemExit('Theme bootstrap is missing')
    theme_text=(BUILD/'inc/Theme.php').read_text(encoding='utf-8')
    settings_text=(BUILD/'inc/ThemeSettings.php').read_text(encoding='utf-8')
    if 'ThemeSettings.php' not in theme_text or 'ThemeSettings::class' not in theme_text:raise SystemExit('ThemeSettings bootstrap is missing')
